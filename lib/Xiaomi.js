...
module.exports = class Xiaomi extends events.EventEmitter {
  ...
  turnOff() {
    this._getGateway().setColor('black', 0);
  }
  ...
  _onMessage(msg, rsinfo) {
    ...
    if (!this.ready) {
      ...
      if (message.cmd === 'get_id_list_ack')
        this._nbDevices = this._nbDevices + JSON.parse(message.data).length;
      if (message.cmd === 'read_ack')
        this._devicesRead++;
      if (message.cmd === 'heartbeat' && message.model === 'gateway')
        this._heartbeat = true;
      if (this._nbDevices + this._nbGateway === this._devicesRead && this._heartbeat) {
        this.ready = true;
        this.emit('ready');
      }
      console.log('this._nbDevices :', this._nbDevices)
      console.log('this._devicesRead :', this._devicesRead)
      console.log('this._nbGateway :', this._nbGateway)
  ...
  _createDevice(message) {
    ...
    switch (message.model) {
      ...
      case 'sensor_cube.aqgl01':
        ...
        break
      case 'weather.v1':
        device = new Weather(message.sid);
        break
      case 'magnet':
        device = new Magnet(message.sid);
        break
      default:
  ...
  _reportDevices() {
    for (let sid in this.devices) {
      ...
      if (answer && this.ready) {
        console.log('cron :', JSON.stringify(answer))
        this.emit(answer.msg, answer.data);
      }
    }
  }
